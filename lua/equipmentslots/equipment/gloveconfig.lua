...
if EquipmentSlots.ShowDebugPrints then
	...
	print( "Including EquipmentSlot Glove Config.")

end

local GlovePieces = GlovePieces || {};
GlovePieces.Database = {};

function GlovePieces.Add( item )

	GlovePieces.Database[#GlovePieces.Database+1] = item;

end

GlovePieces.Add( {
	name = "Mario's Sturdy Gloves";
	attributes = {
		["strength"] = 2;
	};
	Worth = 5;
	UseLength = 30;
	TickUse = false;
	description = "Many sought these very gloves as a treasure of fabled times.";
...
hook.Add( "InitializeEquipment", "RegisterGlovePieces", function( currentEquipment )

	for k , v in pairs( GlovePieces.Database ) do
		...
		EquipmentSlots.RegisterEquipment( v , 9);
