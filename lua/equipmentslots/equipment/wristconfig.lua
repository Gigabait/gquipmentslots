...
if EquipmentSlots.ShowDebugPrints then
	...
	print( "Including EquipmentSlot Wrist config.")

end

local WristPieces = WristPieces || {};
WristPieces.Database = {};

function WristPieces.Add( item )

	WristPieces.Database[#WristPieces.Database+1] = item;

end

WristPieces.Add( {
	name = "Mario's Sturdy Wrist Guards";
	attributes = {
		["armor"] = 5;
	};
	Worth = 45;
	UseLength = 30;
	TickUse = false;
	description = "Many sought these wrist pieces as a treasure of fabled times.";
...
hook.Add( "InitializeEquipment", "RegisterWristPieces", function( currentEquipment )

	for k , v in pairs( WristPieces.Database ) do
		...
		EquipmentSlots.RegisterEquipment( v, 6);
